...
- CSV, 사진 ZIP, FairNote 백업 파일 내보내기는 사용자가 직접 실행하며 Android 공유 화면으로 전달되는 기기 내 작업
...
- `com.harich1.fairnote.DYNAMIC_RECEIVER_NOT_EXPORTED_PERMISSION` (AndroidX가 앱 내부 통신 보호를 위해 자동 생성)
...
9. 실제 Android 기기에서 설치해 사진, 명함, 로컬 저장, CSV·사진 ZIP·FairNote 백업/복원을 확인한다.

신규 개인 개발자 계정에 비공개 테스트 요구사항이 표시되면 Play Console이 안내하는 테스터 수와 기간을 충족한 뒤 프로덕션 액세스를 신청한다.

## 1.0 빌드 검증

- `npm run check`: 통과 (단위 테스트 2개, 브라우저 테스트 10개)
- Android `testReleaseUnitTest`, `lintRelease`, `assembleDebug`, `bundleRelease`: 통과
- AAB JAR 서명: 검증 통과
- 업로드 인증서 만료일: 2053-11-15
- AAB SHA-256: `61ED7D708ED674846BB1677BD5A1A6F1D5F22E96F97DC8502DC35E87BEE4E8EF`
- 실제 Android 기기 설치 검증: Play 내부 테스트 업로드 후 수행
